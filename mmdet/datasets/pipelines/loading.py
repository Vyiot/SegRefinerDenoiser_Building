    ...
    def __call__(self, results):
        ...
        if not self.test_mode:
            ...
            if isinstance(coarse_masks, str):
                ...
                mask = cv2.imread(filename, cv2.IMREAD_GRAYSCALE)
                assert mask is not None, f'Failed to load coarse mask: {filename}'
                # Handle both 0/1 and 0/255 valued masks correctly
...
@PIPELINES.register_module()
class LoadPrecomputedNoisyMask:
    """Load pre-computed noisy mask from file.
    
    Required keys: "noise_mask_path", "timestep".
    Added keys: "object_noisy_masks", "timestep", "object_img", "object_gt_masks".
    """
    def __call__(self, results):
        if 'noise_mask_path' not in results:
            return results
        
        mask_path = results['noise_mask_path']
        # Load the mask
        mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
        assert mask is not None, f'Failed to load noisy mask at {mask_path}'
        
        # Ensure binary 0/1
        mask = (mask > 0).astype(np.uint8)
        
        h, w = mask.shape
        results['object_noisy_masks'] = BitmapMasks([mask], h, w)
        if 'mask_fields' in results and 'object_noisy_masks' not in results['mask_fields']:
            results['mask_fields'].append('object_noisy_masks')

        # Load T-1 mask as training target (xT → xT-1 step-wise prediction)
        if 'noise_mask_prev_path' in results:
            prev_path = results['noise_mask_prev_path']
            prev_mask = cv2.imread(prev_path, cv2.IMREAD_GRAYSCALE)
            assert prev_mask is not None, f'Failed to load prev noisy mask at {prev_path}'
            prev_mask = (prev_mask > 0).astype(np.uint8)
            ph, pw = prev_mask.shape
            results['object_noisy_masks_prev'] = BitmapMasks([prev_mask], ph, pw)
            if 'mask_fields' in results and 'object_noisy_masks_prev' not in results['mask_fields']:
                results['mask_fields'].append('object_noisy_masks_prev')

        # Rename standard fields to object fields as expected by SegRefiner
        if 'img' in results:
            results['object_img'] = results['img']
            if 'img_fields' in results and 'object_img' not in results['img_fields']:
                results['img_fields'].append('object_img')
                
        if 'gt_masks' in results:
            results['object_gt_masks'] = results['gt_masks']
            if 'mask_fields' in results and 'object_gt_masks' not in results['mask_fields']:
                results['mask_fields'].append('object_gt_masks')
...
def modify_boundary(image, regional_sample_rate=0.1, sample_rate=0.1, move_rate=0.0, iou_target=0.9, min_iou=0.5):
    ...
    # Save original for min_iou guard
    gt = np.copy(image)
    ...
    #draw boundary
    image = np.zeros((gt.shape[0], gt.shape[1], 3))
    ...
    # Min-IoU guard: if perturbation was too aggressive, fall back to GT
    final_iou = compute_iou(image, gt)
